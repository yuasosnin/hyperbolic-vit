...
def riemannian_gradient(x, c):
    return RiemannianGradient.apply(x, c)
...
def _dist(x, y, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    dist_c = torch.arctanh(sqrt_c * _mobius_add(-x, y, c).norm(dim=-1, p=2, keepdim=keepdim))
...
def _dist0(x, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    dist_c = torch.arctanh(sqrt_c * x.norm(dim=-1, p=2, keepdim=keepdim))
    return dist_c * 2 / sqrt_c
...
def _logmap0(y, c):
    ...
    return y / y_norm / sqrt_c * torch.arctanh(sqrt_c * y_norm)
...
def _mobius_matvec(m, x, c):
    ...
    res_c = tanh(mx_norm / x_norm * torch.arctanh(sqrt_c * x_norm)) * mx / (mx_norm * sqrt_c)
...
def _hyperbolic_softmax(X: Tensor, A: Tensor, P: Tensor, c):
    ...
    logit = k.unsqueeze(1) * torch.arcsinh(num / denom)
...
def _dist_matrix(x, y, c):
    sqrt_c = c ** 0.5
    return (2 / sqrt_c * torch.arctanh(sqrt_c * _mobius_addition_batch(-x, y, c=c).norm(p=2, dim=-1)))
