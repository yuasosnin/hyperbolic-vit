...
class CARS196DataModule(LightningDataModule):
    def __init__(
            self, dataset_root, n_labels=2, n_instances=10, batch_size=128, num_workers=2):
        super().__init__()
        ...
        if not (self.dataset_root / "devkit").exists():
            download_cars(self.dataset_root)
        df = build_cars196_df(self.dataset_root)
        df_train, df_val, df_test = self._split_train_test(df, method="by_class")

        self.train_dataset = DatasetWithLabels(
            df_train, dataset_root=self.dataset_root, transform=get_augs_albu(im_size=224))
        self.val_dataset = DatasetQueryGallery(
            df_val, dataset_root=self.dataset_root, transform=get_normalisation_resize_albu(im_size=224))
        self.test_datset = DatasetQueryGallery(
            df_test, dataset_root=self.dataset_root, transform=get_normalisation_resize_albu(im_size=224))
    ...
    def _split_train_test(self, df, method="default"):
        if method == "default":
            ...
            df_val = df.loc[train_idx < df["label"] <= train_val_idx]
